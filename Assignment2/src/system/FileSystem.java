...
public class FileSystem {
  ...
  private final String PATHERRORMSG = "Invalid path provided";
  
  /** Error message for a non-existent content */
  private final String NOCONTENTMSG = "Content does not exist";
  
  /** Error message for a non-existent directory */
  private final String NODIRECTORYMSG = "No directory found";
  ...
  public Directory getDir(String path) {
    ...
    else if (path.length() > currDirPath.length()
        && path.equals(path.substring(0, currDirPath.length()))) {
      int pathModifier = 1;
